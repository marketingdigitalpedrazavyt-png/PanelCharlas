...
export const api = {
  ...
  buscarPorDni: (dni) => req("/inscripciones/buscar?dni=" + encodeURIComponent(dni)),
  ...
  actualizarEvento: (id, b) => req("/eventos/" + id, { method: "PUT", body: b, auth: true }),
  cambiarEstadoEvento: (id, activo) => req("/eventos/" + id + "/activo", { method: "PUT", body: { activo }, auth: true }),
  reenviarEvento: (id, soloPendientes) => req("/eventos/" + id + "/reenviar", { method: "POST", body: { soloPendientes }, auth: true }),
